...
def __defineModel(C, D):
    ...
    try: # to load from cache
        ...
        model.add(layers.Flatten(input_shape=( C.dictionary['p_order'], D.numberOfSeries() )))
        model.add(layers.Dense(units=units, kernel_regularizer=regularizers.l1_l2(l1=5e-4, l2=5e-4)))
...
def defineFitPredict(C: Configuration, D:Data, VARIABLES_train:np.array, SERIES_train:np.array, VARIABLES_test:np.array, SERIES_test:np.array):
    ...
    C.dictionary['modelnamePlus'] = C.dictionary['modelname'] + '_dense_define_fit_predict.h5'
